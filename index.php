<?php

  //function.phpを読み込む
  require('function.php');

  $record_span = 12;
  //GETパラメータ
  $current_page = (!empty($_GET['p']) && is_numeric($_GET['p'])) ? $_GET['p'] : 1;
  $word = (!empty($_GET['word'])) ? $_GET['word'] : null;
  $words_logic_flag = (!empty($_GET['words_logic_flag']) && is_numeric($_GET['words_logic_flag'])) ? $_GET['words_logic_flag'] : null;
  $g_id = (!empty($_GET['g_id'])  && is_numeric($_GET['g_id'])) ? $_GET['g_id'] : null;
  $category = (!empty($_GET['category'])) ? $_GET['category'] : null;
  $category_flag = (!empty($_GET['category_flag']) && is_numeric($_GET['category_flag'])) ? $_GET['category_flag'] : null;
  $s_id = (!empty($_GET['s_id']) && is_numeric($_GET['s_id'])) ? $_GET['s_id'] : null;
  $year = (!empty($_GET['year']) && is_numeric($_GET['year'])) ? $_GET['year'] : null;
  
  $logic_flag = (!empty($_GET['logic_flag']) && is_numeric($_GET['logic_flag'])) ? $_GET['logic_flag'] : null;
  $flow_flag = (isset($_GET['flow_flag']) && is_numeric($_GET['flow_flag'])) ? $_GET['flow_flag'] : 0;

  $words = (!empty($word)) ? explode(' ',mb_convert_kana($word,'s')) : null;
  $current_min_record = ($current_page - 1) * $record_span;

  $contents = getSearchContent($words,$words_logic_flag,$g_id,$category,$category_flag,$s_id,$year,null,$logic_flag,$flow_flag,$current_page,$record_span);

  //DBから各属性を読み込む
  $sellers = getSeller();
  $genres = getGenre();
  $categories = getCategory();

  require('login.php');


  $title = '書籍検索';
  require('head.php');
  debugStart();
?>
  <body>
    <p class="c-success-msg js-success-msg"><?php getMsg(); ?></p>
    <?php require('header.php'); ?>
    <div class="p-modal">
      <div class="p-modal__modal-back js-modal-back js-modal-quit"></div>
      <div class="p-modal__modal-window js-modal-window">
        <?php require('modal_contents_search_menu.php'); ?>
      </div>
    </div>
    <main class="l-container">
      <div class="p-search-top">
        <h2 class="p-search-top__page-title c-page-title"><?php echo $title; ?></h2>
        <?php if(!empty($contents['total_record']) && !empty($contents['data'])){ ?>
        <p class="p-search-top__search-result c-search-result"><?php echo $contents['total_record']; ?>件の書籍が見つかりました<br><span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><?php echo $current_page; ?>ページ目&nbsp;&nbsp;<?php echo $current_min_record + 1; ?>&nbsp;-&nbsp;<?php echo count($contents['data']) + $current_min_record; ?>件</p>
        <?php }else{ ?>
        <p class="p-search-top__search-result c-search-result">書籍は見つかりませんでした</p>
        <?php } ?>
        <p class="p-search-top__search-option js-modal-trigger">検索オプション</p>
      </div>
      <div class="l-col2">
        <article class="l-main">
          <div class="p-found-content">
          <?php
              if(!empty($contents)){
                foreach($contents['data'] as $key => $val){
          ?>
            <div class="p-found-content__content-panel c-content-panel">
              <div class="p-found-content__img-wrapper">
                <a href="content_detail.php<?php echo (!empty(keepGETparam())) ? keepGETparam().'&b_id='.$val['b_id'] : '?b_id='.$val['b_id']; ?>" class="p-found-content__img-link"><img src="<?php displayImg($val['pic1']); ?>" alt="書籍" class="p-found-content__img"></a>
              </div>
              <p class="p-found-content__title c-content-panel__title">
                <a href="content_detail.php<?php echo (!empty(keepGETparam())) ? keepGETparam().'&b_id='.$val['b_id'] : '?b_id='.$val['b_id']; ?>" class="p-found-content__title-link c-content-panel__title-link">
                  <?php echo (mb_strlen($val['content_title']) > 27) ? mb_substr(preg_replace('/　|\s+/','', sanitize($val['content_title'])),0,27,'UTF-8').'...' : preg_replace('/　|\s+/','', sanitize($val['content_title'])); ?>
                </a>
              </p>
              <table class="p-found-content__table c-content-panel__table">
                <tr><td class="p-found-content__left-cell"><i class="fa-sharp fa-solid fa-star p-found-content__fonticon c-fonticon"></i></td><td class="p-found-content__right-cell"><?php echo $val['average_rate']; ?></td></tr>
                <tr><td class="p-found-content__left-cell"><i class="fa-sharp fa-solid fa-heart p-found-content__fonticon c-fonticon"></i></td><td class="p-found-content__right-cell"><?php echo $val['fav_num']; ?></td></tr>
                <?php if(empty($com_off)){ ?>
                <tr><td class="p-found-content__left-cell"><i class="fa-sharp fa-solid fa-message p-found-content__fonticon c-fonticon"></i></td><td class="p-found-content__right-cell"><?php echo $val['com_num']; ?></td></tr>
                <?php } ?>
                <tr><td class="p-found-content__left-cell"><i class="fa-sharp fa-solid fa-eye p-found-content__fonticon c-fonticon"></i></td><td class="p-found-content__right-cell"><?php echo $val['visited']; ?></td></tr>
              </table>
            </div>
          <?php
                }
              }
          ?>
          </div>
          <?php
            if(!empty($contents['total_page']) && !empty($contents['data'])){
              pagenation('index.php',$current_page,$contents['total_page']);
            }
          ?>
        </article>
        <aside class="l-sub">
          <?php if(empty(is_Login()) && empty(is_Login(false))){ ?>
          <div class="p-sidebar">
            <form action="" method="post" class="p-sidebar__loginform">
              <div class="p-sidebar__err-msg c-err-msg"><?php echoErr('common'); ?></div>
              <div class="p-sidebar__form-row">
                <label for="login_id" class="p-sidebar__form-label c-form-label c-form-label--side">ログインID</label>
                <div class="l-col1-form__err-msg c-err-msg"><?php echoErr('login_id'); ?></div>
                <input type="text" name="login_id" placeholder="ログインID" id="login_id" class="p-sidebar__textform c-textform c-textform--side <?php is_Err('common'); ?>" value="<?php echo keepTextData($login_id,'login_id'); ?>">
              </div>
              <div class="p-sidebar__form-row">
                <label for="pass" class="p-sidebar__form-label c-form-label c-form-label--side">パスワード</label>
                <div class="l-col1-form__err-msg c-err-msg"><?php echoErr('pass'); ?></div>
                <div class="p-sidebar__passform-wrapper">
                  <input type="password" name="pass" placeholder="パスワード" id="pass" class="p-sidebar__passwordform c-passwordform js-passform c-passwordform js-passform--side<?php is_Err('common'); ?>" value="<?php echo keepTextData($pass,'pass'); ?>">
                  <i class="fa-solid fa-eye p-sidebar__fonticon c-fonticon js-pass-show"></i>
                </div>
              </div>
              <div class="p-sidebar__text">
                <input type="checkbox" name="keep_login" id="keep_login" class="p-sidebar__checkboxform c-checkboxform c-checkboxform--side">
                <label for="keep_login" class="p-sidebar__checkbox-label c-checkboxform__label c-checkboxform__label--side">ログインしたままにする</label>
              </div>
              <input type="submit" value="ログイン" class="p-sidebar__btn c-btn c-btn--active c-btn--l">
              <p class="p-sidebar__text"><a href="signup.php" class="p-sidebar__link">こちらから会員登録できます</a></p>
              <p class="p-sidebar__text"><a href="key_sender.php" class="p-sidebar__link">こちらはパスワード忘れの方</a></p>
            </form>
          </div>
          <?php } ?>
          <?php
            if(!empty(is_Login())){
              require('side_user_menu.php');
            }elseif(!empty(is_Login(false))){
              require('side_admin_menu.php');
            }
          ?>
        </aside>
      </div>
    </main>
<?php
  require('footer.php');
  debugFinish();
?>