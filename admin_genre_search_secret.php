<?php
  ...
  $genres = getEditGenre($words,$words_logic_flag,$flow_flag,$current_page,$record_span);

  if(!empty($_POST)){
    $append_flag = (!empty($_POST['append'])) ? true : false;

    if(!empty($append_flag)){
      $new_genre = array_shift($_POST);
      $new_kana = array_shift($_POST);

      //バリデーション
      //genre !empty 255文字以内
      validMax($new_genre,'common');
      validEnter($new_genre,'common');
      ...
      if(empty($err_msg)){
        ...
        try{
          $dbh = dbConnect();
          $sql = 'INSERT INTO genre (genre,kana,create_date) VALUES (:genre,:kana,:create_date)';
          $data = array(
            ':genre' => $new_genre,
            ':kana' => $new_kana,
      ...
      $new_genre = $renew[(array_search('更新する',array_values($_POST),true) - 2)];
      ...
      if(empty($err_msg)){
        ...
        try{
          $dbh = dbConnect();
          $sql = 'UPDATE genre SET genre = :genre,kana = :kana WHERE g_id = :g_id';
          $data = array(
            ':genre' => $new_genre,
            ':kana' => $new_kana,
            ':g_id' => $target
  ...
  <body>
    ...
    <main class="l-container">
      <div class="p-search-top">
        ...
        <?php if(!empty($genres['total_record']) && !empty($genres['data'])){ ?>
        <p class="p-search-top__search-result c-search-result"><?php echo $genres['total_record']; ?>件のジャンルが見つかりました<br><span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><?php echo $current_page; ?>ページ目&nbsp;&nbsp;<?php echo $current_min_record + 1; ?>&nbsp;-&nbsp;<?php echo count($genres['data']) + $current_min_record; ?>件</p>
        <?php }else{ ?>
        <p class="p-search-top__search-result c-search-result">ジャンルは見つかりませんでした</p>
        <?php } ?>
      ...
      <div class="l-col2">
        <article class="l-main">
          <div class="p-found-record">
            ...
            <form action="" method="post">
              <div class="p-found-record__add-form">
                ...
                <div class="p-found-record__form-wrapper js-opened-area">
                  <input type="text" name="new_genre" placeholder="ジャンル" class="p-found-record__textform c-textform">
              ...
              <div>
              <?php
              if(!empty($genres['data'])){
                foreach($genres['data'] as $key => $val){
              ?>
                <div class="p-found-record__record-row">
                  <span class="p-found-record__record-info c-record-info">◆&nbsp;<?php echo $val['genre']; ?></span>
                  <span class="p-found-record__cmd c-choice-cmd js-open-btn">編集する</span>
                  <div class="p-found-record__form-wrapper js-opened-area">
                    <input type="text" name="<?php echo $val['g_id']; ?>_genre" placeholder="ジャンル" class="p-found-record__textform c-textform">
                    <input type="text" name="<?php echo $val['g_id']; ?>_kana" placeholder="よみがな" class="p-found-record__textform c-textform">
                    <input type="submit" name="<?php echo $val['g_id']; ?>" value="更新する" class="p-found-record__btn c-btn c-btn--active">
                  </div>
                </div>
              <?php
                  }
            ...
            <?php
              if(!empty($genres['total_page']) && !empty($genres['data'])){
                pagenation('admin_genre_search_secret.php',$current_page,$genres['total_page']);
              }
            ?>
          </div>
        </article>
