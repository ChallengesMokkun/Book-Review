<?php
  ...
  if(!empty($_POST)){
    ...
    if(empty($err_msg)){
      //バリデーション
      //古いパスワードが正しい
      validPassword($old_pass,'old_pass',$a_id,false);
      ...
      if(empty($err_msg)){
        ...
        if(empty($err_msg)){
          ...
          try{
            $dbh = dbConnect();
            $sql = 'UPDATE admin SET pass = :pass WHERE a_id = :a_id AND delete_flag = 0';
            $data = array(
      ...
      <div class="l-col1-form">
        ...
        <form action="" method="post" class="p-member-form">
          ...
          <div class="p-member-form__btn-wrapper">
            ...
            <a href="admin_menu_page_secret.php" class="p-member-form__btn c-btn c-btn--inactive c-btn--m">戻る</a>
          </div>
        </form>
      </div>
    </main>
